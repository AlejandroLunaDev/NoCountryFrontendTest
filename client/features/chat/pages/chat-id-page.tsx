...
export function ChatIdPage() {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        console.log(
          `Fetching chats for user ${user?.id}, attempt ${
            queryClient.getQueryState(['chat', chatId])?.fetchStatus ===
            'fetching'
              ? 'in progress'
              : 1
          }`
        ...
        // Después de 1 segundo, quitamos el estado de carga
        setTimeout(() => {
          setIsLoading(false);
        }, 1000);
      }
    ...
    staleTime: 30000, // Consideramos los datos "frescos" por 30 segundos
  ...
  // Estado de carga inicial o mientras se refresca
  ...
  // Mostrar pantalla de error si hay un problema cargando el chat
  if (error && !chat) {
    return (
      <div className='flex h-screen overflow-hidden'>
        ...
        <div className='flex-1 flex flex-col items-center justify-center bg-zinc-900 text-center p-4'>
          ...
          <p className='text-zinc-400 mb-6 max-w-md'>
            No pudimos cargar este chat. Puede que haya sido eliminado o que no
            tengas acceso.
          </p>
          <div className='flex gap-4'>
            ...
            <Button onClick={() => router.push('/chat')}>Volver a chats</Button>
          </div>
        </div>
      </div>
    );
